...
describe('maintenance', () => {
  ...
  it('requires an exported batch and exact phrase before deleting originals', async () => {
    ...
    await expect(cleanOriginals(store, { dataDir: process.cwd(), dbPath: ':memory:', host: '127.0.0.1', port: 3000, corsOrigins: [], ai: null, concurrency: 4 }, '2026-09', 'DELETE ORIGINALS 2026-08')).rejects.toThrow('INVALID_CLEANUP_CONFIRMATION');
    await expect(cleanOriginals(store, { dataDir: process.cwd(), dbPath: ':memory:', host: '127.0.0.1', port: 3000, corsOrigins: [], ai: null, concurrency: 4 }, '2026-09', 'DELETE ORIGINALS 2026-09')).rejects.toThrow('CLEANUP_NOT_ALLOWED');
